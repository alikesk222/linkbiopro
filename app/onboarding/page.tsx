...
function Step1Bio({ onNext }: StepProps & { username: string }) {
  return (
    <div className="text-center">
      ...
      <h2 className="font-display text-2xl font-bold mb-2 text-ink">Hoş geldiniz!</h2>
      <p className="text-ink-soft mb-8">Profilinizi oluşturmak sadece birkaç adım sürer.</p>
      <button onClick={onNext} className="bg-brand-500 hover:bg-brand-600 text-white font-semibold px-8 py-3 rounded-xl transition-colors">
        Başlayalım &rarr;
      </button>
    </div>
  )
  ...
  return (
    <div>
      <div className="text-center mb-8">
        ...
        <h2 className="font-display text-2xl font-bold mb-2 text-ink">Linklerinizi ekleyin</h2>
        <p className="text-ink-soft">Hangi platformlarda aktifsiniz? URL&apos;leri girin.</p>
      </div>
        ...
        {links.map((link, i) => (
          <div key={i} className="flex gap-2">
            <select
              ...
              className="bg-white border border-line rounded-xl px-3 py-2.5 text-sm text-ink focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 w-36 shrink-0"
            >
            ...
            <input
              ...
              type="url"
              className="flex-1 bg-white border border-line rounded-xl px-4 py-2.5 text-sm text-ink placeholder-ink-faint focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
            />
          </div>
        ))}
      </div>
      ...
      {links.length < 5 && (
        <button
          ...
          className="text-sm text-brand-600 hover:text-brand-700 mb-6 block"
        >
          + Link ekle
        </button>
      )}
      ...
      <div className="flex gap-3">
        <button
          onClick={save}
          disabled={saving}
          className="flex-1 bg-brand-500 hover:bg-brand-600 disabled:opacity-40 text-white font-semibold py-3 rounded-xl transition-colors"
        >
          {saving ? 'Kaydediliyor...' : 'Devam Et →'}
        </button>
        <button onClick={onNext} className="text-ink-faint hover:text-ink-soft text-sm px-4 py-3 rounded-xl transition-colors">
          Atla
        </button>
      </div>
    </div>
...
function Step3Theme({ onNext, username }: StepProps & { username: string }) {
  ...
  return (
    <div>
      <div className="text-center mb-8">
        <div className="text-4xl mb-3">🎨</div>
        <h2 className="font-display text-2xl font-bold mb-2 text-ink">Tema seçin</h2>
        <p className="text-ink-soft">Profilinizin görünümünü kişiselleştirin.</p>
      </div>
        ...
        {THEMES.map(t => (
          <button
            ...
            className={`rounded-2xl p-4 border-2 transition-all ${
              theme === t.id ? 'border-brand-500 scale-105 shadow-md shadow-brand-100' : 'border-line hover:border-ink-faint'
            }`}
          >
      ...
      <button
        onClick={save}
        disabled={saving}
        className="w-full bg-brand-500 hover:bg-brand-600 disabled:opacity-40 text-white font-semibold py-3 rounded-xl transition-colors"
      >
...
function Step4Done({ username }: { username: string }) {
  ...
  return (
    <div className="text-center">
      ...
      <h2 className="font-display text-2xl font-bold mb-2 text-ink">Profiliniz hazır!</h2>
      <p className="text-ink-soft mb-2">Profilinizin adresi:</p>
      <div className="bg-brand-50 border border-brand-200 rounded-xl px-4 py-3 font-mono text-brand-600 text-sm mb-8 inline-block">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        <a
          ...
          className="border border-brand-200 text-brand-600 hover:bg-brand-50 font-semibold px-6 py-3 rounded-xl transition-colors"
        >
          Profilimi Gör
        </a>
        <button
            ...
            {[0, 1, 2].map(i => (
              <div
                ...
                className={`h-1.5 rounded-full transition-all ${
                  i <= step ? 'bg-brand-500 w-12' : 'bg-line w-6'
                }`}
              />
            ))}
          </div>
